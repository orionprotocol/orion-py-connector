...
def EIP712_DOMAIN(chId: int):
    return make_domain(name="Orion Exchange", version="1", chainId=chId,
...
def signEIP712Struct(chain_id: int, data: EIP712Struct, private_key: str) -> str:
    ...
    message = data.signable_bytes(EIP712_DOMAIN(chain_id))
    sm = SignableMessage(
        HexBytes(b'\x01'),
        message[2: 34],
        message[34: 66]
    ...
    logging.debug(f'Order Signature: {signature}')
    return signature
...
def hashOrder(data: Order) -> str:
    ...
    logging.debug(f'Order Hash: {hash}')
    return hash
